...
class SupplyChainProvider(Web3Provider):
    def __init__(self):
        ...
        self.contract = self.w3.eth.contract(
            address=settings.ADDRESS_CONTRACT_SUPPLY_CHAIN, abi=factory_abi
        )
    ...
    def listing_product_to_marketplace(self, item_id, product_id, owner, status):
        function = self.contract.functions.listing_product(
            item_id, product_id, owner, status
        )
        tx_hash = self.sign_and_send_transaction(function)
        return tx_hash

    def buy_product_in_market(
        self, product_id, id_trans, buyer, quantity, type_product
    ):
        function = self.contract.functions.buy_product_in_market(
            product_id, quantity, buyer, id_trans, type_product
        )
        tx_hash = self.sign_and_send_transaction(function)
        return tx_hash
